...
public class GestionRepartoJUnitTest {
	 ...
	 *  
	 * <p>Comprueba la inicializaci�n de los atributos.</p>
	 *  
	...
	public void testGestionReparto() {
		...
		assertEquals("El atributo mapa no se inicializ� correctamente", mapa, gestor.getMapa());
		
		assertEquals("El gestor 0 (superior-izquierda) no se inicializ� correctamente.", 
				construirRespuestaEstadoGestor(0,0,0,0), gestor.getEstadoGestorLocalNum(0));
	
		assertEquals("El gestor 1 (superior-derecha) no se inicializ� correctamente.", 
				construirRespuestaEstadoGestor(0,0,0,0), gestor.getEstadoGestorLocalNum(1));
		
		assertEquals("El gestor 2 (inferior-izquierda) no se inicializ� correctamente.", 
				construirRespuestaEstadoGestor(0,0,0,0), gestor.getEstadoGestorLocalNum(2));
		
		assertEquals("El gestor 3 (inferior-derecha) no se inicializ� correctamente.", 
	 ...
	 * 
	 * <p>Presupone que el m�todo add de GestionLocalReparto funciona bien.</p>
	 * 
	 * <p>Se crean transportes y se asignan a distintas localidades controlando a qu�
	...
	public void testAddTransporteLocalidad() {
		...
		for (int i=0;i<NUM_IT;i++){
			...
			for (int j=0;j<4;j++){
				...
				assertEquals("El gestor "+j+" no se actualiz� correctamente.", 
	 ...
	 * y los m�todos de GestorReparto:
	 * constructor y addTransporteLocalidad.</p>
	 * 
	 * <p>Presupone que el m�todo asignarPedido de GestionRepartoLocal funciona bien.</p>
	 *  
	 ...
	 * controlando el n�mero de transportes disponibles y el de pedidos en espera.</p>
	 */
	@Test
	public void testAsignarPedido() {
		...
		for (int i=0;i<NUM_IT;i++){
			for (int j=0;j<4;j++){
				...
				assertEquals("[Iteracion "+i+"] El gestor "+j+" no se actualiz� correctamente.", construirRespuestaEstadoGestor(i>0?0:1,0,i<1?0:i-1,i), gestor.getEstadoGestorLocalNum(j));
				
				gestor.asignarPedido(pedidosPequenos[i][j]);
				
				assertEquals("[Iteracion "+i+"] El gestor "+j+" no se actualiz� correctamente.", construirRespuestaEstadoGestor(0,0,i,i), gestor.getEstadoGestorLocalNum(j));
			}	
		}
	}
	...
	* <p>Utiliza los constructores de Moto, FurgonetaPropia y Pedido,
	 ...
	 * 
	 * <p>Presupone que el m�todo notificarEntregaPedido de GestorRepartoLocal funciona bien.</p>
	 * 
	 * <p>Se crean transportes y se asignan a distintas localidades. 
	 * Se asignan pedidos para motos y furgonetas en distintas localidades
	 * y se notifica la entrega de dichos pedidos
	 * controlando el n�mero de transportes disponibles y el de 
	...
	public void testNotificarEntregaPedido() {
		...
		for (int i=0;i<NUM_IT;i++){
			for (int j=0;j<4;j++){
				...
				assertEquals("[Iteracion "+i+"] El gestor "+j+" no se actualiz� correctamente.", 
		...
		for (int i=0;i<NUM_IT-1;i++){
			for (int j=0;j<4;j++){
				...
				assertEquals("[Iteracion "+i+"] El gestor "+j+" no se actualiz� correctamente.", construirRespuestaEstadoGestor(0,0,NUM_IT-2-i,NUM_IT-2-i), gestor.getEstadoGestorLocalNum(j));
			}	
		}
		for (int j=0;j<4;j++){
			gestor.notificarEntregaPedido(pedidosGrandes[NUM_IT-1][j]);
			
			assertEquals("[Iteracion "+(NUM_IT-1)+"] El gestor "+j+" no se actualiz� correctamente.", construirRespuestaEstadoGestor(0,1,0,0), gestor.getEstadoGestorLocalNum(j));
			
			gestor.notificarEntregaPedido(pedidosPequenos[NUM_IT-1][j]);
			
			assertEquals("[Iteracion "+(NUM_IT-1)+"] El gestor "+j+" no se actualiz� correctamente.", construirRespuestaEstadoGestor(1,1,0,0), gestor.getEstadoGestorLocalNum(j));
		}
	}
